...
import { checkPincode, createOrder, createRazorpayOrder, getCart } from "../api/client";
              ...
              <div className="p-5 md:p-7">
                ...
                {step === 2 && (
                  <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
                    {[
                      ["Today, 3 PM - 6 PM", "Fastest delivery slot", "Rs. 80"],
                      ["Today, 6 PM - 9 PM", "Evening celebration slot", "Rs. 80"],
                      ["Tomorrow, 10 AM - 1 PM", "Scheduled fresh delivery", "Free"]
                    ].map(([title, copy, price], index) => (
                      <label key={title} className={`flex cursor-pointer items-center justify-between gap-4 rounded-lg border p-4 ${index === 0 ? "border-[#e61951] bg-[#fff2e9]" : "border-[#ebebeb] bg-white hover:border-[#e61951]"}`}>
                        <span className="flex items-center gap-3">
                          <input type="radio" name="slot" defaultChecked={index === 0} className="h-4 w-4 accent-[#e61951]" />
                          <span>
                            <span className="block text-sm font-black text-[#1f2221]">{title}</span>
                            <span className="mt-1 block text-xs font-bold text-[#6f7573]">{copy}</span>
                          </span>
                        </span>
                        <span className="shrink-0 text-sm font-black text-[#e61951]">{price}</span>
                      </label>
                    ))}
                    ...
                    <Field label="Delivery Date" name="deliveryDate" type="date" defaultValue={checkoutData.deliveryDate || ""} required />
                  </motion.div>
                )}
...
function Field({ label, name, type = "text", placeholder, required = false, defaultValue = "" }) {
  return (
    <label className="block">
      ...
      <input name={name} type={type} placeholder={placeholder} required={required} defaultValue={defaultValue} className="bk-input h-12 px-4 text-sm" />
    </label>
  );
}
